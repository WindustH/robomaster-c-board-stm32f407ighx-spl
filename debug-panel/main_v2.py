...
Updated debug panel with new configuration system using map.json and panel.json
"""
...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QHBoxLayout,
                           QWidget, QSplitter, QTabWidget, QMessageBox)
from PyQt5.QtCore import Qt

from modules.config_v2 import MonitorConfigV2
from modules.map_parser import MapParser
from modules.data_monitor_v2 import DataMonitorV2
...
from modules.graph_panel_v2 import GraphPanelV2
from modules.control_panel_v2 import ControlPanelV2


class MotorMonitorGUIV2(QMainWindow):
    """Main GUI class for debug panel v2"""

    def __init__(self):
        super().__init__()

        # Initialize configuration
        self.config = MonitorConfigV2()
        self.parser = MapParser(self.config.map_file)
        self.data_monitor = DataMonitorV2(self.config, self.parser)
        self.openocd_manager = OpenOCDManager(self.config)

        # Initialize panels
        self.connection_panel = ConnectionPanel(self.config)
        self.data_source_panel = DataSourcePanel(self.config)
        self.graph_panel = GraphPanelV2(self.config)
        self.control_panel = ControlPanelV2(self.config)
    ...
    def _init_ui(self) -> None:
        ...
        self.setWindowTitle("STM32F407 Motor Control Debug Panel v2")
    ...
    def _connect_to_target(self) -> None:
        ...
        try:
            ...
            # Start OpenOCD if auto-start is enabled
            if self.config.auto_start_openocd:
                if not self.openocd_manager.start():
                    QMessageBox.warning(self, "Connection Error", "Failed to start OpenOCD")
                    return
